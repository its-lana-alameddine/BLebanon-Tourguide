...
if (isset($_POST['submit'])) {
    ...
    if (empty($firstname) || empty($lastname) || empty($email) || empty($password) || empty($confirmPass)) {
        ...
        header ("Location: ../transportations.php?error=invalidfirstname");
        exit();
    } else if (!preg_match("/^[a-zA-Z]*/", $lastname)) {
        header ("Location: ../transportations.php?error=invalidlastname");
        exit();
    } else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($rowCount > 0 ) {
                header ("Location: ../transportations.php?error=accountalreadyexists");
                exit();
            } else {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_execute($stmt);
                    header ("Location: ../transportations.php?success=registered");
                    exit();
                }
            }
        }
